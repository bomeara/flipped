...
library(ggplot2)
nflips <- 10
probs <- data.frame(flip=rep(sequence(nflips),3), prob_heads = c(prob_heads_linear(nflips, slope=0), prob_heads_linear(nflips, slope=0.10), prob_heads_linear(nflips, slope=-0.05)), slope=as.factor(c(rep(0, nflips), rep(0.10, nflips), rep(-0.05,nflips))))
print(ggplot(probs, aes(x=flip, y=prob_heads, colour=slope, group=slope)) + geom_line() + geom_point() + ylab("Probability of heads") + scale_x_continuous(name="Flip", breaks=sequence(nflips)) + scale_colour_brewer(palette = "Dark2") )
```
